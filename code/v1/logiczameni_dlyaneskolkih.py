...
def extract_data_from_old_docx(input_path):
    ...
    rev_pattern = re.compile(r"\bРев\w*", re.IGNORECASE)  # Регулярное выражение для поиска слова, начинающегося на "Рев"
    found_rev = False  # Флаг для отслеживания, нашли ли мы уже ревизию

    # Проверка таблиц в верхнем колонтитуле на наличие srf_number, паспорта и ревизии
        ...
        for table in header.tables:
            for row in table.rows:
                for cell in row.cells:
                    ...
                    # Поиск слова, начинающегося на "Рев"
                    if not found_rev:
                        match = rev_pattern.search(cell.text)
                        if match:
                            # Проверка, есть ли буква после слова "Рев"
                            rev_text = cell.text[match.end():].strip()
                            if rev_text and rev_text[0].isalpha():  # Если есть буква
                                extracted_data["rev_number"] = match.group(0) + " " + rev_text[0]  # Записываем букву
                            else:
                                # Пытаемся взять букву из следующей ячейки, если она существует
                                next_cell_index = row.cells.index(cell) + 1
                                if next_cell_index < len(row.cells):
                                    next_cell = row.cells[next_cell_index]
                                    if next_cell.text and next_cell.text[0].isalpha():
                                        extracted_data["rev_number"] = match.group(0) + " " + next_cell.text[0]  # Записываем букву из следующей ячейки
                            found_rev = True
                            print(f"Найдена ревизия в таблице колонтитула: '{extracted_data['rev_number']}'")  # Отладочное сообщение
    ...
    for para in doc.paragraphs:
        ...
        if para.style.name.startswith('Heading'):
            ...
            if "хранени" in para.text.lower():
                ...
                print("Обнаружен заголовок 'Утилизци'. Начинаем сбор текста.")  # Отладочное сообщение
